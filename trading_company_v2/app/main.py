...
def _build_dashboard_payload(state: CompanyState) -> dict:
    active_desks = set((state.strategy_book or {}).get("active_desks") or settings.active_desk_set)
    closed_positions = [
        {**item, "name": resolve_symbol_name(str(item.get("symbol") or ""), str(item.get("desk") or ""))}
        for item in load_paper_closed_positions(limit=40)
        if str(item.get("desk") or "") in active_desks
    ][:20]
    open_positions = [
        {**item, "name": resolve_symbol_name(str(item.get("symbol") or ""), str(item.get("desk") or ""))}
        for item in (state.open_positions or [])
        if str(item.get("desk") or "") in active_desks
    ]
...
def api_korea_names() -> dict:
    ...
    try:
        ...
        _extras = {
            ...
            "379810": "KODEX 나스닥100", "347850": "디앤디파마텍", "307950": "현대오토에버",
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html,body{margin:0;padding:0}
    ...
    .desk-card.desk-disabled{opacity:.72;border-style:dashed}
...
<div class="app">
  ...
  <div class="table-section">
    <div class="table-head">
      <span class="section-title">전략별 실적</span>
      <span style="font-size:.74rem;color:var(--muted)" id="strategy-stats-label">누적 성과</span>
    </div>
    <div class="table-wrap">
      <div id="strategy-stats-body"><div class="empty-row">실적 데이터 없음</div></div>
    </div>
  </div>
...
<script>
  ...
  function renderDesks(desks){var map=[['korea','dk-korea'],['us','dk-us']];var dg=document.getElementById('desk-grid');map.forEach(function(m){var key=m[0],pfx=m[1],item=(desks||{})[key]||{};var card=dg.querySelector('.desk-card[data-desk="'+key+'"]');if(!card)return;card.style.display='';var cls=actionCls(item.action);card.className='desk-card '+(cls==='watch'?'':''+cls)+(item.disabled?' desk-disabled':'');document.getElementById(pfx+'-act').textContent=item.disabled?'준비중':actionKo(item.action);document.getElementById(pfx+'-act').className='desk-action '+cls;document.getElementById(pfx+'-focus').textContent=item.focus||'신호 없음';var sizeEl=document.getElementById(pfx+'-size');sizeEl.textContent=item.size||'0.00x';sizeEl.className='desk-size'+(item.size&&item.size!=='0.00x'?' active':'');var qfill=document.getElementById(pfx+'-qfill');if(qfill){var qs=parseFloat(item.quality_score||0),qt=parseFloat(item.quality_threshold||0.58);var qpct=qt>0?Math.min(qs/qt*100,100):0;qfill.style.width=qpct.toFixed(1)+'%';qfill.className='mini-gauge-fill'+(qpct>=100?' ready':qpct>=70?' arming':'');var qvalEl=document.getElementById(pfx+'-qval');var qthrEl=document.getElementById(pfx+'-qthr');if(qvalEl)qvalEl.textContent='품질 '+qs.toFixed(2);if(qthrEl)qthrEl.textContent='기준 '+qt.toFixed(2);}
      // Breakout badge for Korea / US desks
  ...
  function renderStrategyStatsCompat(stats){
    ...
    var lbl=document.getElementById('strategy-stats-label');
    if(!body)return;
    if(!stats||!stats.length){
      body.innerHTML='<div class="empty-row">전략별 실적 데이터 없음 (청산 건이 쌓이면 자동 표시)</div>';
      return;
    }
    if(lbl)lbl.textContent='총 '+stats.length+'개 전략';
    var rows=stats.map(function(s){
      var wr=parseFloat(s.win_rate||0);
      var pnl=parseFloat((s.avg_pnl_pct!=null?s.avg_pnl_pct:s.avg_pnl)||0);
      var totalPnl=parseFloat((s.total_pnl_pct!=null?s.total_pnl_pct:s.total_pnl)||0);
      var total=parseInt((s.total_trades!=null?s.total_trades:s.n_trades)||0);
      var wins=parseInt(s.wins||0);
      var losses=parseInt((s.losses!=null?s.losses:Math.max(total-wins,0))||0);
      var wrCls=wr>=60?'pos':wr<40?'neg':'';
      var pnlCls=pnl>0?'pos':pnl<0?'neg':'';
      var totalCls=totalPnl>0?'pos':totalPnl<0?'neg':'';
      var desk=s.desk?'<span style="color:var(--muted);font-size:.72rem"> '+esc(s.desk)+'</span>':'';
      return '<tr>'
        +'<td class="sym" style="font-size:.8rem">'+esc(s.strategy_type||'--')+desk+'</td>'
        +'<td style="text-align:center;font-size:.8rem">'+total+'건</td>'
        +'<td class="'+wrCls+'" style="font-weight:700;text-align:center">'+wr.toFixed(1)+'%</td>'
        +'<td class="'+pnlCls+'" style="font-weight:700;text-align:right">'+fmtPct(pnl)+'</td>'
        +'<td class="'+totalCls+'" style="font-weight:700;text-align:right">'+fmtPct(totalPnl)+'</td>'
        +'<td style="color:var(--muted);font-size:.75rem;text-align:center">'+wins+'W/'+losses+'L</td>'
        +'</tr>';
    }).join('');
    body.innerHTML='<table class="pos-table"><thead><tr><th>전략</th><th style="text-align:center">횟수</th><th style="text-align:center">승률</th><th style="text-align:right">평균손익</th><th style="text-align:right">누적손익</th><th style="text-align:center">W/L</th></tr></thead><tbody>'+rows+'</tbody></table>';
  }
  renderStrategyStats=renderStrategyStatsCompat;
  async function fetchJsonSafe(path, required){
    var res=await fetch(path,{cache:'no-store',credentials:'same-origin'});
    if(!res.ok){
      if(required)throw new Error(path+' HTTP '+res.status);
      console.warn('optional dashboard endpoint failed',path,res.status);
      return null;
    }
    try{return await res.json();}
    catch(e){
      if(required)throw e;
      console.warn('optional dashboard JSON failed',path,e);
      return null;
    }
  }
  async function loadData(){
    try{
      var data=await fetchJsonSafe('/dashboard-data',true);
      var health=await fetchJsonSafe('/health',false)||{};
      var stratStats=await fetchJsonSafe('/api/strategy-stats',false)||[];
      var state=data.state||{},dash=data.dashboard||{},readiness=data.live_readiness_checklist||{},brokerH=data.broker_live_health||{},exec=(dash.execution_summary||{}),perf=(dash.performance||{}),cap=(dash.capital||{}),blockSummary=((dash.exposure||{}).entry_block_summary)||((readiness||{}).entry_block_summary)||{};
      var isLive=String(readiness.execution_mode||'').indexOf('live')>=0;
      var dot=document.getElementById('status-dot');
      dot.className='status-dot'+(blockSummary.blocked?' err':isLive?' ':'');
      var modeEl=document.getElementById('mode-tag');
      modeEl.textContent=String(readiness.execution_mode||'모의투자');
      modeEl.className='mode-tag'+(isLive?' live':'');
      document.getElementById('update-time').textContent=toKST(state.updated_at);
      var ab=document.getElementById('alert-bar');
      if(blockSummary&&blockSummary.blocked){
        ab.textContent='⚠️ '+String(blockSummary.detail||blockSummary.headline||'실행 차단');
        ab.className='alert-bar visible';
      }else{
        ab.textContent='';
        ab.className='alert-bar';
      }
      renderOperatorBriefing(dash.operator_briefing||{});
      renderPnl(perf,cap);
      renderStatusBar(state,readiness,blockSummary);
      renderSignal(dash.crypto_live_lane||null,dash.crypto_live_lane_history||[]);
      window.__deskDrilldown=dash.desk_drilldown||{};
      ...
      renderBroker(brokerH,readiness);
      renderAgentLog(dash.agent_log||[]);
      renderStrategyStats(stratStats||[]);
    }catch(e){
      var dot2=document.getElementById('status-dot');
      dot2.className='status-dot err';
      var ab2=document.getElementById('alert-bar');
      ab2.textContent='⚠️ 데이터 로딩 실패: '+e.message;
...
def scanner_data() -> dict:
    ...
    # ── Korea stock candidates ──
    korea_desk_view = state.desk_views.get("korea_stock_desk", {}) if state.desk_views else {}
    korea_raw = (
        korea_desk_view.get("gap_candidates", [])
...
var KOREA_NAMES = {
  ...
  "347850":"디앤디파마텍","307950":"현대오토에버","006800":"미래에셋증권"
};
...
function renderKoreaTable(){
  ...
  var body=document.getElementById('korea-body');
  var cnt=document.getElementById('korea-cnt');
  sec.style.display='';
  if(!_koreaData||!_koreaData.length){
    cnt.textContent='0';
    body.innerHTML='<tr><td colspan="11" style="text-align:center;padding:24px;color:var(--muted)">현재 표시할 한국 주식 후보가 없습니다. 장중 신호가 나오면 이 영역에 바로 표시됩니다.</td></tr>';
    return;
  }
