...
class KoreaStockDeskAgent(BaseAgent):
    ...
    def run(self) -> AgentResult:
        ...
        enriched_candidates = []
        for item in leaders:
            if float(item.get("gap_pct", 0) or 0.0) < 2.0:
                continue
            ...
            liquidity_score = min(volume, 300000) / 300000 * 0.22 if volume > 0 else 0.0
            overheat_penalty = 0.0
            if gap_pct >= 18.0:
                overheat_penalty += 0.18
            elif gap_pct >= 12.0:
                overheat_penalty += 0.08
            if rsi_value is not None and float(rsi_value) >= 74.0:
                overheat_penalty += 0.12
            if burst_change >= 10.0:
                overheat_penalty += 0.08
            if ema_gap >= 12.0:
        ...
        if avg_signal >= 0.66:
            score += 0.12
        elif avg_signal >= 0.58:
            score += 0.06
        elif avg_signal < 0.48 and gap_candidates:
            score -= 0.08
