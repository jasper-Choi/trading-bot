...
def _candidate_is_hot_entry_eligible(item: dict[str, Any]) -> bool:
    ...
    ssl_sweep_confirmed = bool(item.get("ssl_sweep_confirmed", False))  # ICT 매도측 유동성 스윕 = 스마트머니 매수 신호
    ...
    # EMA 정배열 / RSI 재점화 / SSL 스윕(스마트머니 매수) / 고품질 돌파 시 진입 임계값 완화
    _ema_stack_relax = (
        (0.02 if ema_stack_bullish else 0.0)
        + (0.01 if rsi_reset_confirmed else 0.0)
        + (0.015 if ssl_sweep_confirmed else 0.0)  # SSL 스윕: 매도 유동성 청소 후 상승 = 임계값 완화
    ...
    range_impulse_ok = (
        ...
        and combined >= 0.45  # 0.38 → 0.45: 오진입 감소 (너무 약한 신호 차단)
    ...
    # obvious_trend: [2026-05-18 비활성화]
    # 데이터 근거: 13건 WR 0%, peak=0.000% — 전략 자체가 현재 시장에 맞지 않음.
    # 모든 임계값 강화 시도에도 승률 개선 없음. 백테스트 재검증 전까지 완전 차단.
    # 재활성화 기준: 백테스트 WR >= 55% 확인 후 + stream_ignition + combined >= 0.84
    stream_ignition_val = bool(item.get("stream_ignition", False))
    _ot_stream_score = _float(item.get("stream_score", 0.0))
    obvious_trend_ok = False  # 비활성화: 13건 WR 0% → 손실 차단
