...
## 최신: Claude - 2026-06-16 세션8 (가짜진입가 버그 + 손실 원천 타겟 개선)

### 커밋
- `d21b70f` — 가짜 +359% 진입가 버그 수정 (near_120d 가격맵 + KIS 체결가 entry 보정)
- `a198ecc` — 당일 익절 청산 종목 재진입 차단
- `9399244` — 전략별 켈리 사이징 종목 연동 (sector_wave ×0.40 자동 억제)
- `f99671e` — **손실 타겟 개선: 개장 직후 변동성 회피 + 빠른 컷 시간창 확대**

### 가짜 진입가 버그 (해결)
near_120d가 rotation 시 가격맵 없어 _fetch_live_price 폴백 → 개장 직후 이상틱
(28,355원)을 entry로 기록 → 가짜 +359%. KIS 실거래는 정상가였음.
2겹 수정: near_120d 전용 가격맵 + reconcile에서 avg_fill_price로 entry 보정.
DB 정정 완료(117/119 → 실제 +0.98%/0%), 잔여 비정상 0건.

### 손실 분해 (정정된 45건)
```
rapid_korea_stop  9건 -23.05% (평-2.56%) ← 최대 손실원
봇 전략 최대손실 3건 전부 개장직후(09:00~09:04) 진입직후 급락:
  000150 17초만에 -6.04% / 226950 5초만에 -4.22%
no_momentum_cut -1.33% < rapid_korea_stop -2.56% (빠른컷이 유리)
swing_recovery 0건 발동 (원인 아님)
이기는 거래는 양호: rapid_korea_trail 7건 +21.92%, target 3건 +14%
```

### 타겟 개선 (f99671e)
1. 개장 직후 5분(09:00~09:05 KST) korea 진입 사이즈 ×0.40 (opening_drive 예외)
2. no_momentum_cut 시간창 75초→3분, peak 0.3→0.4 (큰 손절을 빠른 작은 컷으로)

### 정정 후 진짜 성과 (냉정)
순익 +0.28%(본전), 샤프0.24, 켈리 half-0.07(음의 edge), 파산확률 8.2%.
→ 켈리가 음의 edge 자동 억제 중. new_high_breakout(PF1.77)만 양호, 나머지 약함.

### 다음 우선순위
1. 개장 직후 회피·빠른컷 효과 데이터 관찰 (rapid_korea_stop 손실 축소 확인)
2. new_high_breakout 외 약한 전략 정리 지속 (켈리 자동 억제 + 진입조건 강화)
...
2. `82c2125` — desk 성과 판정에서 kis_hold/manual 제외 (NAVER 잔재 -17%가 오발동 유발)
3. `b73cb05` — auto-sell 감사 기록 settled (pending 잔류 → 가드레일 오차단 방지)
4. `5154574`/`29f03ac`/`3d36bd6` — pending 매수 자가 치유 (포지션 존재=체결 증거, ±10분 여유)
5. `1d70c13` — reconcile catch-22 수정 (pending 잔류 시에도 sync/reconcile 실행)
6. `672aac0` — **손실 패턴 3종: 유령 중복 청산 + 하루 2회 재진입 상한 + sector_wave 13:30 마감**

### 06-12 결산
- 기록 -5.29% (16건) — 단, 유령 중복 집계 제거 시 **실손실 약 -1.6%**
- 손실 원인: ① 오후(14시+) 파동 추격 진입 -5.06 ② 동일 종목 4회 재진입 ③ 유령 재청산 이중 집계
- 수익: 오전 트레일 익절 7건 (+1.54 최대) — 오전 진입은 구조적으로 작동
- 차단 버그 연쇄(loss pressure 오발동→pending 고착→catch-22)는 전부 당일 수정·실전 검증 완료

### 다음 세션 확인
1. 유령 재생성 가드: `closed_reason` 무관 30분 차단이 정상 재진입(새 주문)을 막지 않는지
2. daily entry cap: 주문 노트 "daily entry cap: ... 재진입 차단" 발동 확인
3. sector_wave 오전 수익/오후 차단 패턴 데이터 누적 → 13:30 컷오프 타당성 재검증
